...
{ 
	...
	SPI_Init(SPI1, &SPI_InitStructure); 																//初始化SP3
	//启动SPI1
	SPI_Cmd(SPI1,ENABLE); 																						//启动SP3
}
...
void MFRC522Get(char *caidData)
{
	...
	if(status==0)		//如果读到卡
	{
		...
		snprintf(caidData, 7, "%X%X%X%X%X%X", card_readbuf[0],card_readbuf[1],card_readbuf[2],card_readbuf[3],card_readbuf[4],card_readbuf[5]);
		// 设置标志组
		OSFlagPost(&g_flag_grp, FLAG_OLED_RFID_LOCKON + FLAG_LOCKON_FLAG, OS_OPT_POST_FLAG_SET, &err);
	}	
}

void MFRC522Set(u8 *card_pwd)
{
	u8 status,card_size;
	OS_ERR err;
	...
	while(1)
	{
		...
		if(status==0)		//如果读到卡
		{
			...
			card_size=MFRC522_SelectTag(card_numberbuf);	//选卡
			status=MFRC522_Auth(0x60, 4, card_key0Abuf, card_numberbuf);	//验卡
			status=MFRC522_Write(4, card_pwd);		//写卡（写卡要小心，特别是各区的块3）
	...
	if(status==MI_OK)
	{
		...
		OLED_Clear();
		OLED_ShowString(28,25,"success!",12);//6*12
		OLED_Refresh();
		//立即释放互斥锁
	...
	else
	{
		...
		OSMutexPend(&g_mutex_oled,0,OS_OPT_PEND_BLOCKING,NULL,&err);
		OLED_Clear();
		OLED_ShowString(28,25,"failed!",12);//6*12
		OLED_Refresh();
		//立即释放互斥锁
		OSMutexPost(&g_mutex_oled,OS_OPT_POST_NONE,&err);
	}
